...
- **Rate limiting** (`RateLimitingFilter`, Bucket4j, keyed per client IP): `POST /api/auth/login` — 20/min; `POST /api/auth/signup` — 5/min; `POST /api/users` — 5/min; `POST /api/competitions/{id}/start` — 10/min; `PUT /api/competitions/{cid}/matches/{mid}/results` — 30/min. Requests over the limit get `429 Too Many Requests` with a `Retry-After` header and the standard API error envelope. The login limit is deliberately more generous than the others since multiple legitimate users can share one IP logging in within a short window.
